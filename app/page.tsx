...
const translations = {
  zh: {
    ...
    refreshRealtimePrices: "🔄 刷新实时价格 (Together AI)",
    fetchingPrices: "获取中...",
    realtimePriceNote: "点击刷新可获取 Together AI 实时价格",
    subtitle: "智能模型价格对比",
    ...
    categoryAudio: "🎤 音频/语音",
    showingModels: (shown: number, total: number) => `显示 ${shown} / ${total} 个模型`,
    thModel: "模型",
    thProvider: "提供商",
    thInput: "输入 ($/M)",
    thOutput: "输出 ($/M)",
  ...
  en: {
    ...
    refreshRealtimePrices: "🔄 Fetch Live Prices (Together AI)",
    fetchingPrices: "Fetching...",
    realtimePriceNote: "Click to fetch live prices from Together AI",
    ...
    updateBannerSuffix: " · Prices for reference only",
    ...
    costEstimateHint: "Estimated at 50% input + 50% output",
    ...
    showingModels: (shown: number, total: number) => `Showing ${shown} / ${total} models`,
    thModel: "Model",
    thProvider: "Provider",
    thInput: "Input ($/M)",
    thOutput: "Output ($/M)",
...
function formatToday(lang: Lang): string {
  ...
  return new Date().toLocaleDateString(locale, { year: "numeric", month: "long", day: "numeric", weekday: "long" });
}
function formatDateTime(date: Date, lang: Lang): string {
  const locale = lang === "zh" ? "zh-CN" : "en-US";
  return date.toLocaleString(locale, { year: "numeric", month: "2-digit", day: "2-digit", hour: "2-digit", minute: "2-digit", second: "2-digit" });
}
...
function calcTrend(oldPrice: number, newPrice: number): { trend: PriceTrend; pct: number } {
...
function seedInitialTrends(model: Model): LiveModel {
  ...
  return { ...model, inputPrice: inputJitter, outputPrice: outputJitter, inputTrend: inputTrend.trend, outputTrend: outputTrend.trend, inputChangePct: inputTrend.pct, outputChangePct: outputTrend.pct };
}
function matchesUseCaseFilter(model: Model, filter: UseCaseFilter): boolean {
  if (filter === "all") return true;
  const uc = model.useCase.toLowerCase();
  switch (filter) {
    case "coding": return uc.includes("coding") || uc.includes("编程") || model.useCase.includes("Coding");
    case "long-context": return uc.includes("长上下文") || parseContextToTokens(model.context) >= 200_000;
    case "value": return uc.includes("性价比");
    case "fast": return model.speed === "Very Fast" || model.speed === "Fast" || uc.includes("快速");
    default: return true;
  }
}
function monthlyCost(model: { inputPrice: number; outputPrice: number }, millionTokens: number): number {
...
function getStackRecommendations(type: BuildType) {
  if (type === "saas") {
    return {
      layers: [
        { name: "planning", model: "GPT-4o", provider: "OpenAI", costPerMTok: 2.5, monthlyCost: 12.5 },
        { name: "coding", model: "Claude 3.7 Sonnet", provider: "Anthropic", costPerMTok: 3.0, monthlyCost: 15.0 },
        { name: "fastTasks", model: "Gemini 1.5 Flash", provider: "Google", costPerMTok: 0.075, monthlyCost: 0.38 },
        { name: "longContext", model: "Gemini 1.5 Pro", provider: "Google", costPerMTok: 1.25, monthlyCost: 6.25 },
      ],
    };
  } else if (type === "chatbot") {
    return {
      layers: [
        { name: "planning", model: "GPT-4o mini", provider: "OpenAI", costPerMTok: 0.15, monthlyCost: 0.75 },
        { name: "coding", model: "Claude 3 Haiku", provider: "Anthropic", costPerMTok: 0.25, monthlyCost: 1.25 },
        { name: "fastTasks", model: "Gemini 1.5 Flash", provider: "Google", costPerMTok: 0.075, monthlyCost: 0.38 },
        { name: "longContext", model: "DeepSeek V3", provider: "DeepSeek", costPerMTok: 0.14, monthlyCost: 0.70 },
      ],
    };
  } else if (type === "agent") {
    return {
      layers: [
        { name: "planning", model: "GPT-4o", provider: "OpenAI", costPerMTok: 2.5, monthlyCost: 12.5 },
        { name: "coding", model: "Claude 3.7 Sonnet", provider: "Anthropic", costPerMTok: 3.0, monthlyCost: 15.0 },
        { name: "fastTasks", model: "Llama 3.3 70B", provider: "Groq", costPerMTok: 0.7, monthlyCost: 3.5 },
        { name: "longContext", model: "Gemini 1.5 Pro", provider: "Google", costPerMTok: 1.25, monthlyCost: 6.25 },
      ],
    };
    ...
    return {
      layers: [
        { name: "planning", model: "Gemini 1.5 Pro", provider: "Google", costPerMTok: 1.25, monthlyCost: 6.25 },
        { name: "coding", model: "Claude 3.5 Sonnet", provider: "Anthropic", costPerMTok: 3.0, monthlyCost: 15.0 },
        { name: "fastTasks", model: "DeepSeek V3", provider: "DeepSeek", costPerMTok: 0.14, monthlyCost: 0.70 },
        { name: "longContext", model: "Gemini 1.5 Pro", provider: "Google", costPerMTok: 1.25, monthlyCost: 6.25 },
      ],
    };
    ...
    return {
      layers: [
        { name: "planning", model: "Gemini 1.5 Flash", provider: "Google", costPerMTok: 0.075, monthlyCost: 0.38 },
        { name: "coding", model: "Claude 3 Haiku", provider: "Anthropic", costPerMTok: 0.25, monthlyCost: 1.25 },
        { name: "fastTasks", model: "Groq (Llama 3.3 70B)", provider: "Groq", costPerMTok: 0.7, monthlyCost: 3.5 },
        { name: "longContext", model: "Gemini 1.5 Flash", provider: "Google", costPerMTok: 0.075, monthlyCost: 0.38 },
      ],
    };
  } else {
    return {
      layers: [
        { name: "planning", model: "DeepSeek V3", provider: "DeepSeek", costPerMTok: 0.14, monthlyCost: 0.70 },
        { name: "coding", model: "DeepSeek V3", provider: "DeepSeek", costPerMTok: 0.14, monthlyCost: 0.70 },
        { name: "fastTasks", model: "Gemini 1.5 Flash", provider: "Google", costPerMTok: 0.075, monthlyCost: 0.38 },
        { name: "longContext", model: "DeepSeek V3", provider: "DeepSeek", costPerMTok: 0.14, monthlyCost: 0.70 },
      ],
    };
...
export default function Home() {
  ...
  const stackRec = getStackRecommendations(buildType);
  ...
  const fetchRealtimePrices = useCallback(async () => {
    ...
    try {
      const response = await fetch('/api/together-prices');
      const data = await response.json();
      if (data.prices) {
        setLiveModels(prev => prev.map(model => {
          const togetherPrice = data.prices[model.name];
          if (togetherPrice) {
            const newInput = togetherPrice.inputPrice;
            const newOutput = togetherPrice.outputPrice;
  ...
  const refreshSimulatedPrices = useCallback(() => {
    setLiveModels(prev => prev.map(model => {
      ...
      const inputTrend = calcTrend(model.inputPrice, newInput);
      const outputTrend = calcTrend(model.outputPrice, newOutput);
      return { ...model, inputPrice: newInput, outputPrice: newOutput, inputTrend: inputTrend.trend, outputTrend: outputTrend.trend, inputChangePct: inputTrend.pct, outputChangePct: outputTrend.pct };
    }));
    setLastRefresh(new Date());
  }, []);
  ...
  const filteredModels = useMemo(() => {
    ...
    return liveModels.filter(model => {
      ...
      if (!q) return true;
      return model.name.toLowerCase().includes(q) || model.provider.toLowerCase().includes(q) || model.useCase.toLowerCase().includes(q);
    });
  ...
  const costRankings = useMemo(() => {
    if (usageM <= 0) return [];
    return liveModels.map(model => ({ name: model.name, provider: model.provider, cost: monthlyCost(model, usageM) })).sort((a, b) => a.cost - b.cost);
  }, [liveModels, usageM]);
  ...
  const theme = isDark ? {
    ...
    btnSecondary: "bg-gray-800 hover:bg-gray-700 text-gray-200 border border-gray-700",
    ...
    btnSecondary: "bg-gray-100 hover:bg-gray-200 text-gray-700 border border-gray-300",
    ...
    <div className={`min-h-screen transition-colors duration-300 ${theme.page}`}>
      <header className={`border-b ${theme.header}`}>
        <div className="max-w-7xl mx-auto px-4 py-6">
          <div className="flex items-start justify-between gap-4">
            <div><h1 className="text-3xl font-bold bg-gradient-to-r from-blue-500 to-purple-500 bg-clip-text text-transparent">Inferly</h1><p className={`text-sm mt-1 ${theme.muted}`}>{t.tagline}</p></div>
            <div className="flex gap-2">
              <button onClick={() => setLang(l => l === "zh" ? "en" : "zh")} className={`px-3 h-10 rounded-lg border text-sm font-medium ${theme.langToggle}`}>{lang === "zh" ? "English" : "中文"}</button>
              <button onClick={() => setIsDark(d => !d)} className={`flex items-center justify-center w-10 h-10 rounded-lg border ${theme.toggle}`}>{isDark ? <Sun size={18} /> : <Moon size={18} />}</button>
            </div>
          </div>
          ...
          <div className="mt-4 flex flex-wrap items-center gap-3">
            <button onClick={refreshSimulatedPrices} className={`inline-flex items-center gap-2 px-4 py-2 rounded-lg border text-sm font-medium ${theme.refreshBtn}`}><RefreshCw size={16} />{t.refreshPrices}</button>
            <button onClick={fetchRealtimePrices} disabled={isFetchingRealtime} className={`inline-flex items-center gap-2 px-4 py-2 rounded-lg border text-sm font-medium ${theme.btnPrimary}`}><RefreshCw size={16} className={isFetchingRealtime ? "animate-spin" : ""} />{isFetchingRealtime ? t.fetchingPrices : t.refreshRealtimePrices}</button>
            <span className={`text-sm ${theme.muted}`}>{t.lastRefresh}{lastRefresh ? formatDateTime(lastRefresh, lang) : t.notRefreshed}</span>
          </div>
          <p className={`text-xs ${theme.muted} mt-2`}>{t.realtimePriceNote}</p>
        </div>
      </header>
      ...
      <main className="max-w-7xl mx-auto px-4 py-8">
        ...
        <div className="flex flex-col sm:flex-row gap-3 mb-4">
          ...
          <select value={useCaseFilter} onChange={(e) => setUseCaseFilter(e.target.value as UseCaseFilter)} className={`sm:w-52 px-3 py-2.5 rounded-lg border ${theme.select}`} aria-label={t.filterAria}>{USE_CASE_FILTER_LABELS[lang].map((opt) => (<option key={opt.value} value={opt.value}>{t[opt.labelKey]}</option>))}</select>
        </div>
