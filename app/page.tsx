  ...
  return (
    <div className={`min-h-screen transition-colors duration-300 ${theme.page}`}>
      <header className={`border-b ${theme.header}`}>
        <div className="max-w-7xl mx-auto px-4 py-6">
          ...
          <div className={`mt-4 px-4 py-2.5 rounded-lg border text-sm ${theme.updateBanner}`}>
            {t.updateBannerPrefix}{updateDate}{t.updateBannerSuffix}
          </div>
          <div className="mt-4 flex flex-wrap items-center gap-3">
            <button onClick={refreshSimulatedPrices} className={`inline-flex items-center gap-2 px-4 py-2 rounded-lg border text-sm font-medium transition ${theme.refreshBtn}`}>
              <RefreshCw size={16} />{t.refreshPrices}
            </button>
      ...
      <main className="max-w-7xl mx-auto px-4 py-8">
        ...
        {/* 智能推荐引擎 */}
        <div className={`rounded-xl border p-6 mb-8 ${theme.calcSection}`}>
          <h2 className="text-xl font-semibold mb-4 text-center">🤖 {lang === "zh" ? "根据你的需求，智能推荐" : "Smart Recommendations Based on Your Needs"}</h2>
          <p className={`text-sm text-center mb-6 ${theme.muted}`}>{lang === "zh" ? "选择你的首要需求，我们帮你找到最合适的模型" : "Select your priority, we'll find the best model for you"}</p>
          <div className="flex flex-wrap justify-center gap-3 mb-8">
            <button onClick={() => setRecommendType("cheapest")} className={`px-4 py-2 rounded-full transition text-sm font-medium ${recommendType === "cheapest" ? "bg-green-500 text-white" : "bg-green-500/10 text-green-500 border border-green-500/20 hover:bg-green-500/20"}`}>💰 {lang === "zh" ? "最便宜" : "Cheapest"}</button>
            <button onClick={() => setRecommendType("fastest")} className={`px-4 py-2 rounded-full transition text-sm font-medium ${recommendType === "fastest" ? "bg-blue-500 text-white" : "bg-blue-500/10 text-blue-500 border border-blue-500/20 hover:bg-blue-500/20"}`}>⚡ {lang === "zh" ? "最快速度" : "Fastest"}</button>
            <button onClick={() => setRecommendType("coding")} className={`px-4 py-2 rounded-full transition text-sm font-medium ${recommendType === "coding" ? "bg-purple-500 text-white" : "bg-purple-500/10 text-purple-500 border border-purple-500/20 hover:bg-purple-500/20"}`}>🧠 {lang === "zh" ? "最佳编程" : "Best Coding"}</button>
            <button onClick={() => setRecommendType("context")} className={`px-4 py-2 rounded-full transition text-sm font-medium ${recommendType === "context" ? "bg-orange-500 text-white" : "bg-orange-500/10 text-orange-500 border border-orange-500/20 hover:bg-orange-500/20"}`}>📚 {lang === "zh" ? "最长上下文" : "Longest Context"}</button>
          </div>

          {recommendType === "cheapest" && (<div className="grid grid-cols-1 md:grid-cols-2 gap-6"><div className="p-4 rounded-lg bg-gradient-to-r from-green-500/10 to-emerald-500/10 border border-green-500/20"><div className="flex items-center gap-2 mb-2"><span className="text-2xl">🏆</span><span className="font-bold text-green-500">{lang === "zh" ? "🏆 首选：最便宜" : "🏆 Top Pick: Cheapest"}</span></div><div className="text-xl font-semibold">Gemini 1.5 Flash</div><div className="text-sm text-gray-400">Google</div><div className="mt-2 flex gap-3 text-sm"><span className="text-green-500">{lang === "zh" ? "输入" : "Input"}: $0.075/M</span><span className="text-yellow-500">{lang === "zh" ? "输出" : "Output"}: $0.30/M</span></div><div className="text-xs mt-2 text-gray-500">{lang === "zh" ? "💰 性价比之王，适合大规模、成本敏感的应用" : "💰 Best value for large-scale, cost-sensitive applications"}</div></div><div className="p-4 rounded-lg bg-gradient-to-r from-blue-500/10 to-cyan-500/10 border border-blue-500/20"><div className="flex items-center gap-2 mb-2"><span className="text-2xl">🥈</span><span className="font-bold text-blue-500">{lang === "zh" ? "亚军" : "Runner Up"}</span></div><div className="text-xl font-semibold">DeepSeek V3</div><div className="text-sm text-gray-400">DeepSeek</div><div className="mt-2 flex gap-3 text-sm"><span className="text-green-500">{lang === "zh" ? "输入" : "Input"}: $0.14/M</span><span className="text-yellow-500">{lang === "zh" ? "输出" : "Output"}: $0.28/M</span></div><div className="text-xs mt-2 text-gray-500">{lang === "zh" ? "📈 极低的输入成本，强大的通用能力" : "📈 Ultra-low input cost, strong general capability"}</div></div></div>)}
          {recommendType === "fastest" && (<div className="grid grid-cols-1 md:grid-cols-2 gap-6"><div className="p-4 rounded-lg bg-gradient-to-r from-blue-500/10 to-cyan-500/10 border border-blue-500/20"><div className="flex items-center gap-2 mb-2"><span className="text-2xl">🏆</span><span className="font-bold text-blue-500">{lang === "zh" ? "🏆 首选：最快速度" : "🏆 Top Pick: Fastest"}</span></div><div className="text-xl font-semibold">Groq (Llama 3.3 70B)</div><div className="text-sm text-gray-400">Groq</div><div className="mt-2 flex gap-3 text-sm"><span className="text-green-500">{lang === "zh" ? "输入" : "Input"}: $0.70/M</span><span className="text-yellow-500">{lang === "zh" ? "输出" : "Output"}: $0.80/M</span></div><div className="text-xs mt-2 text-gray-500">{lang === "zh" ? "⚡ 专门的推理加速硬件，极低延迟" : "⚡ Dedicated inference acceleration hardware, ultra-low latency"}</div></div><div className="p-4 rounded-lg bg-gradient-to-r from-green-500/10 to-emerald-500/10 border border-green-500/20"><div className="flex items-center gap-2 mb-2"><span className="text-2xl">🥈</span><span className="font-bold text-green-500">{lang === "zh" ? "性价比之选" : "Value Pick"}</span></div><div className="text-xl font-semibold">Gemini 1.5 Flash</div><div className="text-sm text-gray-400">Google</div><div className="mt-2 flex gap-3 text-sm"><span className="text-green-500">{lang === "zh" ? "输入" : "Input"}: $0.075/M</span><span className="text-yellow-500">{lang === "zh" ? "输出" : "Output"}: $0.30/M</span></div><div className="text-xs mt-2 text-gray-500">{lang === "zh" ? "⚡ 快速响应 + 极低成本，最佳平衡" : "⚡ Fast response + ultra-low cost, best balance"}</div></div></div>)}
          {recommendType === "coding" && (<div className="grid grid-cols-1 md:grid-cols-2 gap-6"><div className="p-4 rounded-lg bg-gradient-to-r from-purple-500/10 to-pink-500/10 border border-purple-500/20"><div className="flex items-center gap-2 mb-2"><span className="text-2xl">🏆</span><span className="font-bold text-purple-500">{lang === "zh" ? "🏆 首选：最佳编程" : "🏆 Top Pick: Best Coding"}</span></div><div className="text-xl font-semibold">Claude 3.7 Sonnet</div><div className="text-sm text-gray-400">Anthropic</div><div className="mt-2 flex gap-3 text-sm"><span className="text-green-500">{lang === "zh" ? "输入" : "Input"}: $3.00/M</span><span className="text-yellow-500">{lang === "zh" ? "输出" : "Output"}: $15.00/M</span></div><div className="text-xs mt-2 text-gray-500">{lang === "zh" ? "🧠 公认最强的代码生成和理解能力" : "🧠 Recognized as the strongest code generation and understanding"}</div></div><div className="p-4 rounded-lg bg-gradient-to-r from-blue-500/10 to-cyan-500/10 border border-blue-500/20"><div className="flex items-center gap-2 mb-2"><span className="text-2xl">🥈</span><span className="font-bold text-blue-500">{lang === "zh" ? "全能选手" : "All-Rounder"}</span></div><div className="text-xl font-semibold">GPT-4o</div><div className="text-sm text-gray-400">OpenAI</div><div className="mt-2 flex gap-3 text-sm"><span className="text-green-500">{lang === "zh" ? "输入" : "Input"}: $2.50/M</span><span className="text-yellow-500">{lang === "zh" ? "输出" : "Output"}: $10.00/M</span></div><div className="text-xs mt-2 text-gray-500">{lang === "zh" ? "💻 强大的编程能力 + 多模态支持" : "💻 Strong coding ability + multimodal support"}</div></div></div>)}
          {recommendType === "context" && (<div className="grid grid-cols-1 md:grid-cols-2 gap-6"><div className="p-4 rounded-lg bg-gradient-to-r from-orange-500/10 to-amber-500/10 border border-orange-500/20"><div className="flex items-center gap-2 mb-2"><span className="text-2xl">🏆</span><span className="font-bold text-orange-500">{lang === "zh" ? "🏆 首选：最长上下文" : "🏆 Top Pick: Longest Context"}</span></div><div className="text-xl font-semibold">Gemini 1.5 Pro</div><div className="text-sm text-gray-400">Google</div><div className="mt-2 flex gap-3 text-sm"><span className="text-green-500">{lang === "zh" ? "输入" : "Input"}: $1.25/M</span><span className="text-yellow-500">{lang === "zh" ? "输出" : "Output"}: $5.00/M</span></div><div className="text-xs mt-2 text-gray-500">{lang === "zh" ? "📚 200万 Token 上下文，可处理整本书" : "📚 2M token context, can process entire books"}</div></div><div className="p-4 rounded-lg bg-gradient-to-r from-purple-500/10 to-pink-500/10 border border-purple-500/20"><div className="flex items-center gap-2 mb-2"><span className="text-2xl">🥈</span><span className="font-bold text-purple-500">{lang === "zh" ? "多模态之选" : "Multimodal Pick"}</span></div><div className="text-xl font-semibold">Gemini 2.5 Pro</div><div className="text-sm text-gray-400">Google</div><div className="mt-2 flex gap-3 text-sm"><span className="text-green-500">{lang === "zh" ? "输入" : "Input"}: $1.25/M</span><span className="text-yellow-500">{lang === "zh" ? "输出" : "Output"}: $10.00/M</span></div><div className="text-xs mt-2 text-gray-500">{lang === "zh" ? "🎯 100万 Token + 多模态理解" : "🎯 1M token context + multimodal understanding"}</div></div></div>)}
          <p className="text-center text-xs text-gray-500 mt-6">💡 {lang === "zh" ? "点击上方按钮，推荐内容会动态变化" : "Click the buttons above to see dynamic recommendations"}</p>
        </div>
