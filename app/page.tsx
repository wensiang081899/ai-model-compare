    ...
    <div className={`min-h-screen transition-colors duration-300 ${theme.page}`}>
      <header className={`border-b ${theme.header}`}>
        <div className="max-w-7xl mx-auto px-4 py-6">
          ...
          <div className="mt-4 flex flex-wrap items-center gap-3"><button onClick={refreshSimulatedPrices} className={`inline-flex items-center gap-2 px-4 py-2 rounded-lg border text-sm font-medium ${theme.refreshBtn}`}><RefreshCw size={16} />{t.refreshPrices}</button><span className={`text-sm ${theme.muted}`}>{t.lastRefresh}{lastRefresh ? formatDateTime(lastRefresh, lang) : t.notRefreshed}</span></div>
        </div>
      </header>
